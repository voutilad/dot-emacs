...
** Fonts
   If running in say, [[http://man.openbsd.org/cwm][cwm]],
   we need to set some fonts otherwise it
   assumes it can figure them out from =~/.Xdefaults=...
	...
	(if (eq system-type 'windows-nt)
	  ...
	  (cond ((string-prefix-p "minmin" (system-name))
		...
		((string-prefix-p "kogelvis" (system-name))
		 (set-frame-font "Comic Code 13"))
		((string-prefix-p "kogelvis2" (system-name))
		 (set-frame-font "Comic Code 13"))
		(t (set-frame-font "Comic Code 7")))))
    #+END_SRC
...
*** Line Numbering
	...
	(add-hook 'linum-mode-hook
		  (lambda ()
		    (if (or (string-prefix-p "minmin" (system-name))
